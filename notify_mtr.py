...
push_config = {
    ...
    'PUSH_KEY': 'SCT18330T4IFHX1TXldTJfJygmVaTV45x',                     # server 酱的 PUSH_KEY，兼容旧版与 Turbo 版
...
def telegram_bot(title: str, content: str) -> None:
    ...
    if push_config.get("TG_PROXY_HOST") and push_config.get("TG_PROXY_PORT"):
        if push_config.get("TG_PROXY_AUTH") is not None and "@" not in push_config.get("TG_PROXY_HOST"):
            author=push_config.get("TG_PROXY_AUTH").split(':')
            author[1]=urllib.parse.quote(author[1])
